...
{
    sudo docker run --privileged -itd --name S11TEN${TENANT_ID} ubuntu:sahilfinal
    ...
    sudo docker run --privileged -itd --name S12TEN${TENANT_ID} ubuntu:sahilfinal
    ...
    sudo docker run --privileged -itd --name T11TEN${TENANT_ID} ubuntu:sahilfinal
    ...
    sudo docker run --privileged -itd --name T12TEN${TENANT_ID} ubuntu:sahilfinal
...
assign_static_public_ips_and_iptables(){
	...
	sudo ip netns exec TEN${TENANT_ID} iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ten${TENANT_ID}veth1 -d ${TENANT_PUBLIC_IP} -j DNAT --to-destination 192.168.130.2
    	sudo ip netns exec TEN${TENANT_ID} iptables -t nat -I POSTROUTING  -d ${REMOTE_TENANT_PUBLIC_IP} -j SNAT --to ${TENANT_DNAT_PUBLIC_IP}
	sudo ip netns exec TEN${TENANT_ID} iptables -w -t nat -I POSTROUTING  -d 192.168.130.2 -j MASQUERADE
        # GRE Remote route
	...
	sudo ip netns exec TEN${TENANT_ID} ifconfig lo up
        ...
        sudo ip netns exec TEN${TENANT_ID} iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ten${TENANT_ID}veth1 -d ${REMOTE_TENANT_PUBLIC_IP} -j DNAT --to-destination 192.168.135.2
	sudo ip netns exec TEN${TENANT_ID} iptables -w -t nat -I POSTROUTING  -d 192.168.135.2 -j MASQUERADE
...
adding_tcp_masquerade_rules_to_lbs11(){
    if [ "${HYPERVISOR_FLAG}" = "1" ];then
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.130.2 -m statistic --mode nth --every 4 --packet 0 -j DNAT --to-destination 192.168.80.51
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.130.2 -m statistic --mode nth --every 4 --packet 1 -j DNAT --to-destination 192.168.80.52
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.130.2 -m statistic --mode nth --every 4 --packet 2 -j DNAT --to-destination 192.168.80.53
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.130.2 -m statistic --mode nth --every 4 --packet 3 -j DNAT --to-destination 192.168.80.54
    else
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.135.2 -m statistic --mode nth --every 4 --packet 0 -j DNAT --to-destination 192.168.80.51
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.135.2 -m statistic --mode nth --every 4 --packet 1 -j DNAT --to-destination 192.168.80.52
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.135.2 -m statistic --mode nth --every 4 --packet 2 -j DNAT --to-destination 192.168.80.53
        sudo ip netns exec ${TENANT_ID}_NSLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_nslb11veth1 -d 192.168.135.2 -m statistic --mode nth --every 4 --packet 3 -j DNAT --to-destination 192.168.80.54
    fi
}
...
adding_tcp_masquerade_rules_to_tenant_namespace(){
     ...
     if [ "${HYPERVISOR_FLAG}" = "1" ];then
        sudo ip netns exec TEN${TENANT_ID} iptables -t nat -I PREROUTING -p tcp -m state --state NEW  -i ten${TENANT_ID}veth1 -d ${TENANT_PUBLIC_IP} -m statistic --mode nth --every 2 --packet 0 -j DNAT --to-destination 192.168.130.2

        sudo ip netns exec TEN${TENANT_ID} iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ten${TENANT_ID}veth1 -d ${TENANT_PUBLIC_IP} -m statistic --mode nth --every 2 --packet 1 -j DNAT --to-destination ${REMOTE_TENANT_PUBLIC_IP}
    else
        sudo ip netns exec TEN${TENANT_ID} iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ten${TENANT_ID}veth1 -d ${REMOTE_TENANT_PUBLIC_IP} -j DNAT --to-destination 192.168.135.2
...
{
    if [ "${HYPERVISOR_FLAG}" = "1" ];then
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_appbr13veth1 -d 192.168.85.2 -m statistic --mode nth --every 4 --packet 0 -j DNAT --to-destination 192.168.90.51
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_appbr13veth1 -d 192.168.85.2 -m statistic --mode nth --every 4 --packet 1 -j DNAT --to-destination 192.168.90.52
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -d 192.168.85.2 -m statistic --mode nth --every 4 --packet 2 -j DNAT --to-destination 192.168.90.53
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_appbr13veth1 -d 192.168.85.2 -m statistic --mode nth --every 4 --packet 3 -j DNAT --to-destination 192.168.90.54
    else
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_appbr13veth1 -d 192.168.85.3 -m statistic --mode nth --every 4 --packet 0 -j DNAT --to-destination 192.168.90.51
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_appbr13veth1 -d 192.168.85.3 -m statistic --mode nth --every 4 --packet 1 -j DNAT --to-destination 192.168.90.52
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_appbr13veth1 -d 192.168.85.3 -m statistic --mode nth --every 4 --packet 2 -j DNAT --to-destination 192.168.90.53
        sudo ip netns exec ${TENANT_ID}_EWLB11 iptables -t nat -I PREROUTING -p tcp -m state --state NEW -i ${TENANT_ID}_appbr13veth1 -d 192.168.85.3 -m statistic --mode nth --every 4 --packet 3 -j DNAT --to-destination 192.168.90.54
    fi
}
...
{
	echo "Turning on collectd service"
	...
	sudo docker cp /tmp/${TENANT_ID}/collectd.conf  ${TENANT_ID}_EWLB11:/etc/collectd/
...
{
	echo "create_bridge_and_create_veth_pair ${HYPERVISOR_FLAG}"
	...
	## create veth pair 
	if [ "${HYPERVISOR_FLAG}" = "1" ];then
		echo "In 1_vxlan_fntend"
		sudo ip link add ${TENANT_ID}_vx_fnt type veth peer name ${TENANT_ID}_fnt_vx
		sudo ip link set dev ${TENANT_ID}_vx_fnt up
		sudo ip link set dev ${TENANT_ID}_fnt_vx up
		sudo brctl addif ${TENANT_ID}_br14 ${TENANT_ID}_vx_fnt
		sudo brctl addif bridge43 ${TENANT_ID}_fnt_vx
	fi
